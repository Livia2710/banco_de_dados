...
SELECT professores.nome, cursos.titulo FROM professores INNER JOIN cursos ON professores.curso_id = cursos.id GROUP BY professores.nome;
```
...
SELECT  cursos.titulo AS "Materias", COUNT(alunos.curso_id) AS "Quantidade de Alunos" FROM alunos INNER JOIN cursos ON alunos.curso_id = cursos.id GROUP BY Materias  ORDER BY COUNT(alunos.curso_id) DESC;
```
